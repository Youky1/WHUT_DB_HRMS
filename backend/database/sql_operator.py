...
''' 
    ...
    首先查询该id是否已存在于Staff表，
        ...
        若存在，返回True
'''
def signUpPermissionTest(db,**kwargs):
    # 获取已注册用户的账户ID和密码
    account_datas = search_table_info(db, "staff")
...
def signUp(db,**kwargs):
    ...
    for key, value in kwargs.items():
        ...
        if key == "id" or key == 'sex' or key == 'name' or key == 'Email' or key == 'phone':
            User_info[key] = value
    if insert_alltable_info(db, "account", **account) and insert_alltable_info(db, "user_info", **User_info):  # 插入数据
        return True
    else:
        return False
