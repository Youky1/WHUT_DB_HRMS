...
if __name__ == '__main__':
    ...
    """ 创建表 """
    pysql.create_table_head(db,"account", Account)  # 创建表
    pysql.create_table_head(db, "staff", Staff)  # 创建表
    pysql.create_table_head(db, "department", Department)  # 创建表
    pysql.create_table_head(db, "Employee", Employee)  # 创建表
    pysql.create_table_head(db, "Position2", Position)  # 创建表
    pysql.create_table_head(db, "user_info", user_info)  # 创建表

    """ 插入数据 """
    pysql.insert_alltable_info(db, "account","SQL_testdata//Account.txt")    # 插入数据
    pysql.insert_alltable_info(db, "staff", "SQL_testdata//Staff.txt")  # 插入数据
    pysql.insert_alltable_info(db, "user_info", "SQL_testdata//user_info.txt")  # 插入数据
    ...
    pysql.insert_alltable_info(db, "position2","SQL_testdata//Position.txt")    # 插入数据
